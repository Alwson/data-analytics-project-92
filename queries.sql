...
with days as
	(select
  		CONCAT(employees.first_name,' ', employees.last_name) as seller,
  		extract(isodow from sales.sale_date) as day_of_week,
  		FLOOR(SUM(sales.quantity*products.price)) as income
  		from sales
	left join employees on sales.sales_person_id = employees.employee_id 
	left join products on sales.product_id  = products.product_id 
...
select
  	days.seller as seller,
  	 (case
	  		when days.day_of_week=1 then 'monday'
	  		when days.day_of_week=2 then 'tuesday'
	  		when days.day_of_week=3 then 'wednesday'
	  		when days.day_of_week=4 then 'thursday'
	  		when days.day_of_week=5 then 'friday'
	  		when days.day_of_week=6 then 'saturday'
	  		when days.day_of_week=7 then 'sunday'
	  	Else 'error' END) as day_of_week,
  	FLOOR(SUM(days.income)) as income
  from days
  group by seller, day_of_week
	order by days.day_of_week, seller;
